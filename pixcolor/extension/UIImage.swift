...
extension UIImage {
    ...
    func flippedHorizontally() -> UIImage {
        return UIImage(cgImage: self.cgImage!, scale: CGFloat(1.0), orientation: .upMirrored)
    }

    func flippedVertically() -> UIImage {
        return UIImage(cgImage: self.cgImage!, scale: CGFloat(1.0), orientation: .leftMirrored)
    }
    ...
    func getPixelColorEx(pos: CGPoint) -> UIColor {
    ...
    func areaAverage(pos:CGPoint, deta:CGFloat) -> UIColor {
        var bitmap = [UInt8](repeating: 0, count: 4)
        ...
        if #available(iOS 9.0, *) {
            ...
            let inputImage: CIImage = ciImage ?? CoreImage.CIImage(cgImage: cgImage!)
            let extent = inputImage.extent
            let inputExtent = CIVector(x: extent.origin.x + pos.x - deta/2, y: extent.origin.y + pos.y - deta, z: deta, w: deta)
            let filter = CIFilter(name: "CIAreaAverage", withInputParameters: [kCIInputImageKey: inputImage, kCIInputExtentKey: inputExtent])!
            let outputImage = filter.outputImage!
            let outputExtent = outputImage.extent
            assert(outputExtent.size.width == 1 && outputExtent.size.height == 1)
    ...
    func averageColor(pos:CGPoint, deta:CGFloat) -> UIColor {
        ...
        for row in Int(pos.y - deta/2)...Int(pos.y + deta/2) {
            var rowPtr = rawPixelData! + bytesPerRow * row
            for _ in Int(pos.x - deta/2)...Int(pos.x + deta/2) {
                red    += Int(rowPtr[0])
                green  += Int(rowPtr[1])
                blue   += Int(rowPtr[2])
                rowPtr += Int(stride)
